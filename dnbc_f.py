...
#             self.__add_obs_to_off_buff(cur_obs)
    ...
    def __set_static_gaus_pmfs(self):
        ...
        if (np.sum(cal_med == 127) > 0) | (np.sum(np.isnan(cal_med)) > 0):
            sys.stderr.write('\nAt least one link has a median of 127 or is nan\n\n')
#             quit()
...
#         
#         # these are the possible deltas a person can shift by from the current pixel
...
#             
#             # A place to store if the pixels are neighbors of the current pixel
#             neighbor_idx = np.zeros(self.num_states,dtype='bool')
#             
#             # loop through all shifts in pixel coordinates
#             for ss in shifts:
#                 
#                 # apply the shift
#                 neighbor = cur_pixel + ss
#                 
#                 # Make sure the shift is a valid pixel (takes care of corners)
...
#                 neighbor_idx[0] = True
#             
#             # set probability of transition for only the neighboring pixels
#             self.A_mat[pp,neighbor_idx] = 1./neighbor_idx.sum()

    # set up pi vector and A matrix
    def __init_mats(self):
        
        min_trans_prob = 0.00001
