...
class GHMM_V:
    ...
    def __init__(self,image_obj, rss_editor_obj, network,V,min_p,p127,ltb_len,circ_buff_len,Delta,eta,omega):
        ...
        self.init_counter = 0
        self.circ_buff_len = circ_buff_len
        ...
        self.b_vec = circBuffClass.myCircBuff(circ_buff_len,self.num_states)
        self.delta = circBuffClass.myCircBuff(circ_buff_len,self.num_states)
        self.psi = circBuffClass.myCircBuff(circ_buff_len,self.num_states)
        self.q_star = np.zeros(circ_buff_len)
        self.msrt_history = circBuffClass.myCircBuff(circ_buff_len,self.num_links)
    ...
    def observe(self,cur_line,true_loc = np.array([np.nan,np.nan])):
        ...
        if self.is_updated:
            ...
            # Add RSS vector to measurement history
            self.__add_msrt_to_circ_buff(cur_obs)
            
            # Get likelihoods of current vector observation
            self.__update_b_vec(cur_obs)
            
            # Get delta and psi values
            self.__update_delta_psi()
            
            # Get gamma values
            self.__update_q_star()
    ...
    # Add the current RSS vector measurement to the circular buffer
    def __add_msrt_to_circ_buff(self,cur_obs):
        self.msrt_history.add_observation(cur_obs)
    ...
    def __update_b_vec(self,cur_obs):
        ...
        #scale_val = np.prod(self.var_mat,axis=0)
#         scale_val = 1.0/np.prod(self.var_mat,axis=0)
        ...
        self.b_vec.add_observation(tmp)
        

            
    # This method computes the delta and psi values in the viterbi algorithm
    def __update_delta_psi(self):
        # create the first alpha values when there is only one observation
        if self.is_first_obs:
            deltatmp = self.pi_vec*self.b_vec.get_observation(1)
            deltatmp = deltatmp/np.sum(deltatmp)
            self.delta.add_observation(deltatmp)
            
            psitmp = np.zeros(self.num_states)
            self.psi.add_observation(psitmp)
            
            self.is_first_obs = 0
        
        # create the next alpha values when there is more than one observation
        else:
            delta_mat = np.tile(self.delta.get_observation(1),(self.num_states,1)).T
            b_mat = np.tile(self.b_vec.get_observation(1),(self.num_states,1))
            
            tmpdelta = np.max(self.A_mat*delta_mat*b_mat,axis=0)
            self.delta.add_observation(tmpdelta/tmpdelta.sum())
            
            self.psi.add_observation(np.argmax(self.A_mat*delta_mat,axis=0))
            
            if self.psi.is_full():
                self.psi_is_full = 1
    
    # This method computes q* in the viterbi algorithm
    def __update_q_star(self):
        
        if self.psi_is_full:
            start_idx = 1
            end_idx = self.circ_buff_len-1
            
            while start_idx <= self.circ_buff_len:
                
                if start_idx == 1:
                    self.q_star[end_idx] = np.argmax(self.delta.get_observation(start_idx))
                else:
                    self.q_star[end_idx] = self.psi.get_observation(start_idx-1)[self.q_star[end_idx+1]]
    ...
    def __get_image_est(self):
        ...
        if self.psi_is_full:
            ...
            self.current_loc_est = self.image_obj.pixel_coords_subset[self.q_star[0],:]
            
            # Create the current image
            tmp_img = np.zeros(self.image_obj.num_pixels_all)
            tmp_img[self.image_obj.master_pixel_int_idx] = self.delta.get_observation(self.circ_buff_len)
...
#         var_off = np.minimum(np.maximum(self.off_buff.get_nanvar(),self.omega),5.0)
