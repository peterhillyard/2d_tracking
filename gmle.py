...
class GMLE:
    ...
    def __update_b_vec(self,cur_obs):
        ...
        scale_val = 0.5*self.num_links*np.log(2*np.pi) + 0.5*np.log(self.var_mat).sum(axis=0)
        self.b_vec = scale_val + -0.5*np.sum(((cur_obs_mat - self.mew_mat)**2)/self.var_mat,axis=0)
        
        
#         scale_val = 1.0/np.prod(self.var_mat,axis=0)
#         scale_val[np.isnan(scale_val)] = 0
#         
...
#             self.b_vec = self.b_vec/self.b_vec.sum()
    ...
    def __set_static_gaus_pmfs(self):
        ...
        self.mew_mat = np.tile(mew_off,(self.num_states,1)).T*(self.image_obj.link_pixel_mat_subset == False)
        self.mew_mat += np.tile(mew_on,(self.num_states,1)).T*self.image_obj.link_pixel_mat_subset
        
        self.var_mat = np.tile(var_off,(self.num_states,1)).T*(self.image_obj.link_pixel_mat_subset == False)
