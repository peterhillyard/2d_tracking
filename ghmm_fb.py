...
class GHMM_FB:
    ...
    def __init__(self,image_obj, rss_editor_obj, network,V,min_p,p127,ltb_len,circ_buff_len,Delta,eta,omega):
        ...
        self.alpha = circBuffClass.myCircBuff(circ_buff_len,self.num_states)
        self.msrt_history = circBuffClass.myCircBuff(circ_buff_len,self.num_links)
        self.beta = np.zeros((self.num_states,circ_buff_len))
        self.gamma = np.zeros((self.num_states,circ_buff_len))
    ...
    def __update_alpha(self):
        ...
        if self.is_first_obs:
            alphatmp = self.pi_vec*self.b_vec.get_observation(1)
            alphatmp = alphatmp/np.sum(alphatmp)
            self.alpha.add_observation(alphatmp)
            self.is_first_obs = 0
        
        # create the next alpha values when there is more than one observation
        else:
            alphatmp = np.dot(self.alpha.get_observation(1),self.A_mat)*self.b_vec.get_observation(1)
            self.alpha.add_observation(alphatmp/np.sum(alphatmp))
            
    # Compute the backward (beta) values
    def __update_beta(self): 
        counter_down = self.beta_idx
        counter_up = 0
        while counter_down >= 0:
            if counter_down == self.beta_idx:
                self.beta[:,counter_down] = 1.0
            else:
                tmp = np.dot(self.A_mat,self.b_vec.get_observation(counter_up)*self.beta[:,counter_down+1])
                self.beta[:,counter_down] = tmp/tmp.sum()
            
            counter_down -= 1
            counter_up += 1
        
        self.beta_idx = np.minimum(self.beta_idx + 1,self.circ_buff_len-1)
        
    # Compute the gamma values
    def __update_gamma(self):
        if self.gamma_idx == (self.circ_buff_len-1):
            self.gamma_is_full = 1
        
        counter_down = self.gamma_idx
        counter_up = 1
        while counter_down >= 0:
            numer = self.alpha.get_observation(counter_up)*self.beta[:,self.beta_idx-1]
            denom = np.sum(self.alpha.get_observation(counter_up)*self.beta[:,self.beta_idx-1])
            self.gamma[:,counter_down] = numer/denom
            
            counter_down -= 1
            counter_up += 1
            
        self.gamma_idx = np.minimum(self.gamma_idx + 1,self.circ_buff_len-1)
    
    # Update the current image and location estimate
    def __get_image_est(self):
        
        if self.gamma_is_full:
            # Get the pixel with the smallest hamming distance        
            self.current_loc_est = self.image_obj.pixel_coords_subset[np.argmax(self.gamma[:,0]),:]
            
            # Create the current image
            tmp_img = np.zeros(self.image_obj.num_pixels_all)
            tmp_img[self.image_obj.master_pixel_int_idx] = self.gamma[:,0]
